package com.anxin.appinfosystem.controller.developer;

import com.alibaba.fastjson.JSONArray;
import com.anxin.appinfosystem.pojo.*;
import com.anxin.appinfosystem.service.appInfo.AppInfoService;
import com.anxin.appinfosystem.service.appcategory.AppcategroyService;
import com.anxin.appinfosystem.service.appversion.AppversionService;
import com.anxin.appinfosystem.service.datadictionary.DatadictionaryService;
import com.anxin.appinfosystem.tools.Constants;
import com.anxin.appinfosystem.tools.Fileupload;
import com.anxin.appinfosystem.tools.PageSupport;
import org.apache.commons.io.FilenameUtils;
import org.apache.log4j.Logger;
import org.springframework.http.HttpRequest;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.multipart.MultipartFile;
import org.springframework.web.multipart.support.MultipartFilter;

import javax.annotation.Resource;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpSession;
import javax.validation.Valid;
import java.io.File;
import java.io.IOException;
import java.util.Date;
import java.util.HashMap;
import java.util.List;

@Controller
@RequestMapping("/app")
public class DevAppController {
    //日志
    private Logger logger=Logger.getLogger(DevIndexController.class);

    @Resource
    //APP状态信息的service
    private AppInfoService appInfoService;

    public Logger getLogger() {
        return logger;
    }

    public void setLogger(Logger logger) {
        this.logger = logger;
    }

    @Resource
    //APP分类的service
    private AppcategroyService appcategroyService;

    public AppcategroyService getAppcategroyService() {
        return appcategroyService;
    }

    public void setAppcategroyService(AppcategroyService appcategroyService) {
        this.appcategroyService = appcategroyService;
    }

    @Resource
    //查询字典的方法 (里面放的 app状态..所属平台..)
    private DatadictionaryService datadictionaryService;

    public DatadictionaryService getDatadictionaryService() {
        return datadictionaryService;
    }

    public void setDatadictionaryService(DatadictionaryService datadictionaryService) {
        this.datadictionaryService = datadictionaryService;
    }

    @Resource
    //APP版本各种操作的service
    private AppversionService appversionService;

    public AppInfoService getAppInfoService() {
        return appInfoService;
    }

    public void setAppInfoService(AppInfoService appInfoService) {
        this.appInfoService = appInfoService;
    }

    /**
     * 转发到app信息查询页面
     * @return
     */
    @RequestMapping("/appinfolist.html")
    public String getAppinfoList(Model model,
                                 HttpSession session,
                                 @RequestParam(value="querySoftwareName",required=false) String querySoftwareName,
                                 @RequestParam(value="queryStatus",required=false) String _queryStatus,
                                 @RequestParam(value="queryCategoryLevel1",required=false) String _queryCategoryLevel1,
                                 @RequestParam(value="queryCategoryLevel2",required=false) String _queryCategoryLevel2,
                                 @RequestParam(value="queryCategoryLevel3",required=false) String _queryCategoryLevel3,
                                 @RequestParam(value="queryFlatformId",required=false) String _queryFlatformId,
                                 @RequestParam(value="pageIndex",required=false) String pageIndex){
        logger.info("getAppInfoList -- > querySoftwareName: " + querySoftwareName);
        logger.info("getAppInfoList -- > queryStatus: " + _queryStatus);
        logger.info("getAppInfoList -- > queryCategoryLevel1: " + _queryCategoryLevel1);
        logger.info("getAppInfoList -- > queryCategoryLevel2: " + _queryCategoryLevel2);
        logger.info("getAppInfoList -- > queryCategoryLevel3: " + _queryCategoryLevel3);
        logger.info("getAppInfoList -- > queryFlatformId: " + _queryFlatformId);
        logger.info("getAppInfoList -- > pageIndex: " + pageIndex);
        //获取当前用户的devid
        Integer devId=((DevUser)(session.getAttribute(Constants.DEV_USER_SESSION))).getId();
        List<AppInfo> appInfoList=null;
        //获取app用户状态
        List<DataDictionary> statusList=null;
        //获取app所属平台
        List<DataDictionary> flatFormList=null;
        //列出一级分类列表，注：二级和三级分类通过ajax异步获取
        List<AppCategory> categoryLevel1List=null;
        List<AppCategory> categoryLevel2List=null;
        List<AppCategory> categoryLevel3List=null;
        //页面容量
        Integer pageSize=Constants.pageSize;
        //当前页码
        Integer currentPageNo=1;
        //判断开始的页数是否为空
        if(pageIndex!=null){
            try{
                currentPageNo=Integer.valueOf(pageIndex);
            }catch (Exception e){
                e.printStackTrace();
            }
        }
        //当前的APP状态
        Integer queryStatus=null;
        if(_queryStatus!=null&&!("").equals(_queryStatus)){
            queryStatus=Integer.parseInt(_queryStatus);
        }
        Integer queryCategoryLevel1 = null;
        if(_queryCategoryLevel1 != null && !_queryCategoryLevel1.equals("")){
            queryCategoryLevel1 = Integer.parseInt(_queryCategoryLevel1);
        }
        Integer queryCategoryLevel2 = null;
        if(_queryCategoryLevel2 != null && !_queryCategoryLevel2.equals("")){
            queryCategoryLevel2 = Integer.parseInt(_queryCategoryLevel2);
        }
        Integer queryCategoryLevel3 = null;
        if(_queryCategoryLevel3 != null && !_queryCategoryLevel3.equals("")){
            queryCategoryLevel3 = Integer.parseInt(_queryCategoryLevel3);
        }
        Integer queryFlatformId = null;
        if(_queryFlatformId != null && !_queryFlatformId.equals("")){
            queryFlatformId = Integer.parseInt(_queryFlatformId);
        }
        //总数量（列表）
        int totalCount = 0;
        try {
            totalCount = appInfoService.getAppInfoCount(querySoftwareName, queryStatus, queryCategoryLevel1, queryCategoryLevel2, queryCategoryLevel3, queryFlatformId, devId);
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        //总页数
        PageSupport pages = new PageSupport();
        pages.setCurrentPageNo(currentPageNo);
        pages.setPageSize(pageSize);
        pages.setTotalCount(totalCount);
        int totalPageCount = pages.getTotalPageCount();
        //控制首页和尾页
        if(currentPageNo < 1){
            currentPageNo = 1;
        }else if(currentPageNo > totalPageCount){
            currentPageNo = totalPageCount;
        }
        try {
            appInfoList = appInfoService.getAppInfoList(querySoftwareName, queryStatus, queryCategoryLevel1, queryCategoryLevel2, queryCategoryLevel3, queryFlatformId, devId, currentPageNo, pageSize);
            statusList = datadictionaryService.findValueName("APP_STATUS");
            flatFormList = datadictionaryService.findValueName("APP_FLATFORM");
            categoryLevel1List = appcategroyService.findCategory(null);
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        model.addAttribute("appInfoList", appInfoList);
        model.addAttribute("statusList", statusList);
        model.addAttribute("flatFormList", flatFormList);
        model.addAttribute("categoryLevel1List", categoryLevel1List);
        model.addAttribute("pages",pages);
        model.addAttribute("queryStatus", queryStatus);
        model.addAttribute("querySoftwareName", querySoftwareName);
        model.addAttribute("queryCategoryLevel1", queryCategoryLevel1);
        model.addAttribute("queryCategoryLevel2", queryCategoryLevel2);
        model.addAttribute("queryCategoryLevel3", queryCategoryLevel3);
        model.addAttribute("queryFlatformId", queryFlatformId);
        model.addAttribute("totalCount", totalCount);
        model.addAttribute("totalPageCount", totalPageCount);
        model.addAttribute("currentPageNo",currentPageNo);
        //二级分类列表和三级分类列表---回显
        if(queryCategoryLevel2 != null && !queryCategoryLevel2.equals("")){
            categoryLevel2List = getCategoryList(queryCategoryLevel1.toString());
            model.addAttribute("categoryLevel2List", categoryLevel2List);
        }
        if(queryCategoryLevel3 != null && !queryCategoryLevel3.equals("")){
            categoryLevel3List = getCategoryList(queryCategoryLevel2.toString());
            model.addAttribute("categoryLevel3List", categoryLevel3List);
        }
        return "developer/appinfolist";
    }


    /**
     * ajax方法以父级id查询子级
     * @param pid
     * @return
     */
    @RequestMapping(value = "categorylist.json",method = RequestMethod.GET)
    @ResponseBody
    public List<AppCategory> getAppCategoryList(@RequestParam String pid){
        if(pid.equals("")){
            pid=null;
        }
        System.out.println("==============="+pid);
        Integer spid=pid==null?null:Integer.parseInt(pid);
        List<AppCategory> categoryList=categoryList=appcategroyService.findCategory(spid);
        return categoryList;
    }

    /**
     * 根据父级id查询
     * @param pid
     * @return
     */
    public List<AppCategory> getCategoryList(String pid){
        List<AppCategory> categoryList=null;
        Integer spid=pid==null?null:Integer.parseInt(pid);
        categoryList=appcategroyService.findCategory(spid);
        return categoryList;
    }


    /**
     * 根据typeCode查询出相应的数据字典列表
     * @param tcode
     * @return
     */
    @RequestMapping(value="/datadictionarylist.json",method=RequestMethod.GET)
    @ResponseBody
    public List<DataDictionary> getDataDicList (@RequestParam String tcode){
        List<DataDictionary> dataDictionaryList = null;
        dataDictionaryList = datadictionaryService.findValueName(tcode);
        return dataDictionaryList;
    }


    /**
     * 转发到app添加页面
     * @return
     */
    @RequestMapping("/appinfoadd.html")
    public String appinfoadd()
    {
        return "developer/appinfoadd";
    }

    /**
     * 添加app的方法
     * @param appInfo
     * @param request
     * @param attach
     * @return
     */
    @RequestMapping("/addappinfo")
    public String appinfoAdd(AppInfo appInfo, HttpServletRequest request,
                             @RequestParam(value = "logofile",required = false) MultipartFile attach)
    {
            String logoPicPath=null;
            String logoLocPath=null;
        HashMap<String,Object> path=new HashMap<String, Object>();

        if(!Fileupload.fileupload(attach,request,path,1)){
            request.setAttribute("appInfo",appInfo);
            return "developer/appinfoadd";
        }
            Integer devid=((DevUser)request.getSession().getAttribute(Constants.DEV_USER_SESSION)).getId();
        appInfo.setCreatedBy(devid);
        appInfo.setCreationDate(new Date());
        appInfo.setLogoPicPath(path.get("logoPicPath").toString());
        appInfo.setLogoLocPath(path.get("logoLocPath").toString());

        appInfo.setDevId(devid);
        appInfo.setStatus(1);
        //System.out.println(appInfo);
        try {
            if(appInfoService.add(appInfo)==1){
                return "redirect:appinfolist.html";
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "developer/appinfoadd";
    }

    /**
     * 转发的app修改页面并根据appid查询app的信息
     * @param id
     * @param model
     * @return
     */
    @RequestMapping("/appinfomodify.html")
    public String appinfomodify(@RequestParam("id") String id,Model model){
        AppInfo appInfo=null;
        logger.debug(id+"======================");
        try {
            appInfo=appInfoService.getAppInfo(Integer.parseInt(id),null);
            System.out.println(appInfo);
        } catch (Exception e) {
            e.printStackTrace();
        }
        model.addAttribute("appInfo",appInfo);
        return "developer/appinfomodify";
    }

    /**
     * 修改app信息的方法
     * @param appInfo
     * @param request
     * @param attach
     * @return
     */
    @RequestMapping("/updateInfo")
    public String updateAppInfo(AppInfo appInfo, HttpServletRequest request,
                                @RequestParam(value = "logofile",required = false) MultipartFile attach){
        System.out.println("参数是空的吗?"+appInfo);
        String logoPicPath=null;
        String logoLocPath=null;
        HashMap<String,Object> path=new HashMap<String, Object>();

        if(!Fileupload.fileupload(attach,request,path,1)){
            request.setAttribute("appInfo",appInfo);
            System.out.println("fffffffffffffffffffff"+appInfo);
            return "developer/appinfomodify";
        }
        Integer devid=((DevUser)request.getSession().getAttribute(Constants.DEV_USER_SESSION)).getId();
        appInfo.setModifyBy(devid);
        appInfo.setModifyDate(new Date());
        appInfo.setLogoPicPath(path.get("logoPicPath")==null?null:path.get("logoPicPath").toString());
        appInfo.setLogoLocPath(path.get("logoLocPath")==null?null:path.get("logoLocPath").toString());

        appInfo.setDevId(devid);
        appInfo.setStatus(1);
        System.out.println(appInfo);
        try {
            if(appInfoService.modify(appInfo)==1){
                return "redirect:appinfolist.html";
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "developer/appinfomodify";
    }






    /**
     * 转发查看APP信息页面
     * @return
     */
    @RequestMapping("/appinfoview.html")
    public String appinfoview(@RequestParam("id")String id,Model model)
    {
        AppInfo appInfo=null;
        List<AppVersion> appVersionList=null;
        logger.debug(id+"======================");
        try {
            appInfo=appInfoService.getAppInfo(Integer.parseInt(id),null);
            appVersionList= appversionService.appversionInfo(Integer.parseInt(id));
            System.out.println(appInfo);
        } catch (Exception e) {
            e.printStackTrace();
        }
        model.addAttribute("appInfo",appInfo);
        model.addAttribute("appversion",appVersionList);
        return "developer/appinfoview";
    }


    /**
     * 判断APKName是否唯一 ajax方法
     * @param APKName
     * @return
     */
    @RequestMapping(value="/apkexist.json")
    @ResponseBody
    public Object apkNameIsExit(@RequestParam String APKName){
        HashMap<String, String> resultMap = new HashMap<String, String>();
        if(APKName==null){
            resultMap.put("APKName", "empty");
        }else{
            AppInfo appInfo = null;
            try {
                appInfo = appInfoService.getAppInfo(null, APKName);
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
            if(null != appInfo)
                resultMap.put("APKName", "exist");
            else
                resultMap.put("APKName", "noexist");
        }
        return JSONArray.toJSONString(resultMap);
    }


    /**
     * 删除图片的方法 ajax
     * @param flag
     * @param id
     * @return
     */
    @RequestMapping("/delfile.json")
    @ResponseBody
    public Object delfile(@RequestParam(value = "flag",required = false)String flag,
                          @RequestParam(value = "id",required = false)String id){
        HashMap<String,String> resultMap=new HashMap<String, String>();
        String fileLocPath=null;
        if(flag==null||flag.equals("")||id==null||id.equals("")){
            resultMap.put("result","failed");
        }else if(flag.equals("logo")){//删除logo图片 (操作app_info)
            try {
                fileLocPath=(appInfoService.getAppInfo(Integer.parseInt(id),null).getLogoLocPath());
                File file=new File(fileLocPath);
                if(file.exists()){
                    if(file.delete()){//删除服务器存储的物理文件
                        if(appInfoService.deleteAppLogo(Integer.parseInt(id))==1){//更新表
                            resultMap.put("result","success");
                        }
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
        }else if(flag.equals("apk")){//删除apk文件（操作app_version）
            try {
                fileLocPath = (appversionService.getAppVersionById(Integer.parseInt(id))).getApkLocPath();
                File file = new File(fileLocPath);
                if(file.exists())
                    if(file.delete()){//删除服务器存储的物理文件
                        if(appversionService.deleteApkFile(Integer.parseInt(id))==1){//更新表
                            resultMap.put("result", "success");
                        }
                    }
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
        }
        return JSONArray.toJSONString(resultMap);
    }

    /**
     * 转发到APP版本更新页面
     * @return
     */
    @RequestMapping("/appversionadd.html")
    public String appversionadd(@RequestParam("id")String id,Model model){
        List<AppVersion> appVersionList=null;
        appVersionList=appversionService.appversionInfo(Integer.parseInt(id));
        model.addAttribute("appVersionList",appVersionList);
        model.addAttribute("appId",id);
        return "developer/appversionadd";
    }



    @RequestMapping("/addversion")
    public String addVersion(@Valid AppVersion appVersion, HttpServletRequest request,
                             @RequestParam(value = "apkFile",required = false) MultipartFile attach){
        HashMap<String,Object> apkFilePath=new HashMap<String, Object>();
        if(!Fileupload.fileupload(attach,request,apkFilePath,2)){
            request.setAttribute("appVersion",appVersion);
            return "developer/appversionadd";
        }
        appVersion.setApkLocPath(apkFilePath.get("logoLocPath").toString());
        appVersion.setDownloadLink(apkFilePath.get("logoPicPath").toString());
        Integer devid=((DevUser)request.getSession().getAttribute(Constants.DEV_USER_SESSION)).getId();
        appVersion.setCreatedBy(1);
        appVersion.setCreationDate(new Date());
        appVersion.setPublishStatus(3);
        appVersion.setApkFileName("124124");
        if(appversionService.addVersion(appVersion)==1){
            Integer versionid=appversionService.getVersion(appVersion.getAppId());
            try {
                appInfoService.updateVersionId(versionid,appVersion.getAppId());
                System.out.println("!!!!!!!!@@@@@@@@@@@@@@@@@@@"+appVersion);
                System.out.println("!!!!!!!!!!!!!!@@@@@@@@@@@"+versionid);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return "redirect:appinfolist.html";
        }
        return "developer/appversionadd";
    }


    @RequestMapping("/appVerSionModify.html")
    public String modifyVersion(@RequestParam("id")String id,Model model){
        List<AppVersion> appVersionList=null;
        appVersionList=appversionService.appversionInfo(Integer.parseInt(id));
        AppVersion appVersion=appVersionList.get(0);
        model.addAttribute("appVersion",appVersion);
        model.addAttribute("appVersionList",appVersionList);
        model.addAttribute("appId",id);
        return "developer/appversionmodify";
    }

    //public String


}
